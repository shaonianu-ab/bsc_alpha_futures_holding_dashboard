...
async function copyContractAddress(contract) {
  try {
    ...
    // Use the selection fallback when the browser denies Clipboard API access.
  }

  const fallback = document.createElement("textarea");
  fallback.value = contract;
  ...
  fallback.remove();
  if (!copied) throw new Error("浏览器未允许复制合约地址");
}
  ...
  try {
    ...
    if (type === "manual-holding") {
      ...
      if (result.auto_matched) {
        showNotice("已自动匹配 BSC 合约，余额已计入总持仓。");
...
document.addEventListener("click", async (event) => {
  ...
  if (!target) return;
  if (target.dataset.action === "copy-contract") {
    try {
      await copyContractAddress(target.dataset.contractAddress);
      target.classList.add("copied");
      window.setTimeout(() => target.classList.remove("copied"), 1_500);
      showNotice("合约地址已复制。");
    } catch (error) {
      showNotice(error.message, true);
    }
